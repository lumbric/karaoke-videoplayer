...
# 2. Create data folders and add your MP4 files
mkdir -p data/videos data/covers

# Put your MP4 karaoke videos in "data/videos", file names will be used for metadata search queries
...
By default, the app assumes these paths:
  - Video file: `data/videos/{filename}.mp4`
  - Cover image: `data/covers/{filename}.jpg`

To override default paths for single entries (e.g., different file extensions), use these optional fields:
...
**Video files:**
Karaoke video files stored in `data/videos/`. Ideally named `Artist - Title.mp4` since the filename is used for metadata searches.

**Cover art:**
JPEG images in `data/covers/`. Downloaded automatically by `update-song-data.py` when internet is available. The app shows a default cover icon when no cover image exists.
